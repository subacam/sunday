// Vercel 서버리스 함수판 place-info "핀 신고" 증가 API. 로직은 day4/server.js의
// handlePostPlaceInfoReport와 동일하되, Vercel의 req.body/res.status().json() 인터페이스에
...
module.exports = async (req, res) => {
  ...
  try {
    const upstreamRes = await fetch(supabaseUrl + "/rest/v1/rpc/increment_place_info_report", {
      method: "POST",
      headers: {
        apikey: apiKey,
    ...
    if (!upstreamRes.ok) {
      console.error("[api] place-info 신고 실패, 상태: " + upstreamRes.status);
    ...
    res.status(200).json({ reportCount: result });
  } catch (err) {
    console.error("[api] place-info 신고 호출 실패:", err.message);
