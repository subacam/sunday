// Vercel 서버리스 함수판 place-info 삭제 API. 로직은 day4/server.js의
// handleDeletePlaceInfo와 동일하되, Vercel의 req.body/res.status().json() 인터페이스에
...
// 신뢰 모델이다(로그인이 없어 "누구 것인지" 자체를 서버가 알 방법이 없다).
...
module.exports = async (req, res) => {
  ...
  try {
    const upstreamRes = await fetch(supabaseUrl + "/rest/v1/rpc/delete_place_info_entry", {
      method: "POST",
      headers: {
        apikey: apiKey,
    ...
    if (!upstreamRes.ok) {
      console.error("[api] place-info 삭제 실패, 상태: " + upstreamRes.status);
      res.status(502).json({ error: "UPSTREAM_ERROR", message: "삭제하지 못했습니다." });
      return;
    }
    const deleted = await upstreamRes.json();
    if (!deleted) {
    ...
    console.error("[api] place-info 삭제 호출 실패:", err.message);
