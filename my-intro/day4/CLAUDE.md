...
- 검색 결과 카드·인기 랭킹 카드·가게 상세 페이지(`place.html`) 헤더 세 곳 모두 우상단에
  담기(즐겨찾기) 아이콘 버튼이 있다 — 로그인 필요, 클릭으로 담기/취소 토글, 이미 담아둔
  가게는 진입 즉시 자동으로 "담김" 상태로 표시된다. 세 곳 모두 로직은 같지만 `place.html`은
  자체 완결 원칙 때문에 별도로 복붙 구현했다. 자세한 내용은 `DESIGN.md` §6.15/§6.12와 이
  문서의 "`place_saves` 테이블" 절 참고.
